...
export const SubmissionCard = (article) => {
    ...
    return (
        <div className="my-3 rounded-xl bg-white p-4 flex flex-col space-y-4" style={{ boxShadow: '0px 0px 39px -4px rgba(0, 0, 0, 0.19)' }}>
            <img className="rounded-xl" src={src}></img>
            <div className="text-lg font-bold text-left">{article?.article?.title}</div>
            <div className="mx-8 rounded-xl border border-black py-2 text-xl font-bold cursor-pointer" onClick={() => history.push(`/article/${article.article?._id}`)}>VIEW</div>
        </div>
    );
};
